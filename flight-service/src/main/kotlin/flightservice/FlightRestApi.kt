...
class FlightRestApi(
    ...
    fun getAll(
            ...
            @ApiParam("Location to travel from")
            @RequestParam("fromLocation", required = false)
            fromLocation: String?,

            @ApiParam("Location to travel to")
            @RequestParam("toLocation", required = false)
            toLocation: String?

    ): ResponseEntity<WrappedResponse<PageDto<FlightDto>>> {


        val flights = service.getFlightList(50, fromLocation, toLocation)
        ...
        if (fromLocation != null) builder.queryParam("fromLocation", fromLocation)

        if (toLocation != null) builder.queryParam("toLocation", toLocation)
    ...
    fun mergePatch(@ApiParam("The unique id of the flight")
        ...
        if (jsonNode.has("date")) {
            val dateNode = jsonNode.get("date")
            if (dateNode.isNull) {
                return RestResponseFactory.userFailure("Missing date")
            } else if (dateNode.isTextual) {
                newDate = dateNode.asText()
            } else {
                //Invalid JSON. Non-string name
                return ResponseEntity.status(400).build()
            }
